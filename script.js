...
let numCorSelecionada = 0;

function atualizarCorSelecionada() {
    const opcaoCorSelecionada = document.querySelector
        ('[name="opcao-cor"]:checked').id.charAt(0);
...
function atualizarTamanho() {
    ...
    tituloProduto.innerText = "Pulseira loop esportiva " +
        opcoesCores[numCorSelecionada].toLowerCase() + " para caixa de " + tamanhoCaixa;
...
function atualizarImagemSelecionada() {
    ...
    imagemVisualizacao.src = "./imagens/opcoes-cores/imagens-" +
        opcoesCores[numCorSelecionada].toLowerCase() + "/imagem-" +
